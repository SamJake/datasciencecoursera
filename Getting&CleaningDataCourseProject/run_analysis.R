...
dir3 <- "C:/Users/sam.jacob/Documents/data/UCI HAR Dataset/train"
dir4 <- "C:/Users/sam.jacob/Documents/data/UCI HAR Dataset/test"
...
setwd("C:/Users/sam.jacob/Documents/data/UCI HAR Dataset")
...
cols[1:2] <- TRUE
selected <- total[,cols]
names(selected) <- sapply(names(selected),function(x) {gsub("\\- | \\( | \\) | \\,","",as.character(x))})

selected2 <- tbl_df(selected)
by_person <- group_by(selected2,labels,Subject)
summarize(by_person,mean(by_person$tBodyAcc-mean()-X))
